...
shinyServer(function(input, output) { 
  ...
  make_plot <- function(){ 
    ...
    if( ncol(df)== 7 ) {
      plotEuler(df[-1,1:6], df[-1,]$N_Pts, names(df[-1,1:6]))
      
    } 
    if (ncol(df)==6){
      plotEuler(df[-1,1:5], df[-1,]$N_Pts, names(df[-1,1:5]))
    }
    if (ncol(df)==5){
        plotEuler(df[-1,1:4], df[-1,]$N_Pts, names(df[-1,1:4]))
    }
    if (ncol(df) < 5){
      plot(1,1,labels = FALSE, tck = FALSE); text(1,1,"Use Venn diagram for Invision patients app\n @  https://tanzir.shinyapps.io/venn_for_invision_patients/ " ,col="red",cex = 2)
    }
